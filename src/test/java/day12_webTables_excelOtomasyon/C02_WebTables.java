...
public class C02_WebTables extends TestBase {
    ...
    public void test01(){
        ...
        List<WebElement> sutunSayisi = driver.findElements(By.xpath("(//div[@role='row'])[1]/div"));
        System.out.println("Tablodaki sutun sayisi: " + sutunSayisi.size());

        // 8. Tablodaki 3.kolonda dolu olan elementleri yazdirin
        System.out.println("************ 3.Kolon Elementleri ************");
        List<WebElement> ucuncuSutunElementleri = driver.findElements(By.xpath("//div[@role='row']/div[3]"));

        for (WebElement eachUcuncuSutun:ucuncuSutunElementleri
             ) {
            if (!eachUcuncuSutun.getText().isBlank()){
                System.out.println(eachUcuncuSutun.getText());
            }
        }
